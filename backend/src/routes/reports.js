...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await ReportsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await ReportsDBApi.findAll(req.query);
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'title', 'date_generated'];
      const opts = { fields };
      try {
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await ReportsDBApi.findBy({ id: req.params.id });
