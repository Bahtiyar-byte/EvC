...
import { loadColumns } from './configureAppointmentsCols';
...
const TableSampleAppointments = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (appointmentsNotify.showNotification) {
      notify(
        appointmentsNotify.typeNotification,
        appointmentsNotify.textNotification,
      );
    }
  }, [appointmentsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/appointments/appointments-view/?id=${id}`);
  };
...
export default TableSampleAppointments;
