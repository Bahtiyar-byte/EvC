...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/documents/documentsSlice';
...
const TableSampleDocuments = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (documentsNotify.showNotification) {
      notify(
        documentsNotify.typeNotification,
        documentsNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/documents/documents-view/?id=${id}`);
  };
